...
impl SNum {
    fn repl(&self, v: SNum) {
        let mut_ref: &mut SNum = unsafe {std::mem::transmute( self as *const SNum)};
        *mut_ref = v;
    }
    ...
    fn _explode<'a>(&'a self, depth: i32, exploded: &mut Option<SNum>, left: &mut Option<&'a SNum>, right: &mut Option<&'a SNum>) {
        match self {
            SNum::Regular(x) => {
                if exploded.is_none() {
                    *left = Some(self)
            ...
            SNum::Pair(pair) => if depth == 0 && exploded.is_none() {
                match &**pair {
...
fn parse_chars(input: &mut Chars) -> SNum {
    let c = input.next().unwrap();
    if c.is_digit(10) {
        SNum::Regular(c.to_digit(10).unwrap())
    } else if c == '[' {
        let left = parse_chars(input);
        assert_eq!(input.next().unwrap(), ',');
        let right = parse_chars(input);
        assert_eq!(input.next().unwrap(), ']');
...
fn test_split3() {
    ...
    assert!(n.split());
    assert_eq!(n, SNum::pair(SNum::pair(SNum::Regular(5), SNum::Regular(5)), SNum::Regular(10)));
}
...
fn test_split4() {
    ...
    assert!(n.split());
    assert_eq!(n, SNum::pair(SNum::Regular(1), SNum::pair(SNum::Regular(5), SNum::Regular(5))));
}

#[test]
fn test_magnitude() {
    assert_eq!(parse("[[[[8,7],[7,7]],[[8,6],[7,7]]],[[[0,7],[6,6]],[8,7]]]").magnitude(), 3488)
}
